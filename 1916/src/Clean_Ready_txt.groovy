...
<<<<<<< HEAD:Laurium-1930/Laurium-1930-project/src/entryConstruct.groovy
    ...
    *As this code is, it will not handle any errors of hill when it should be h111
     ...
     */
    // Digit on the right
    matches  =  line =~ /\sbdsi\s(\d)/
    matches.each{m, d -> line = (line =~ "${m}").replaceFirst(" h1"+d) }
    matches  =  line =~ /\sresi\s(\d)/
    matches.each{m, d -> line = (line =~ "${m}").replaceFirst(" r1"+d) }
    matches  =  line =~ /bdsl(\d|O|l)/
    matches.each{m, d -> line = (line =~ "${m}").replaceFirst("h1"+d) }
    matches  =  line =~ /resl(\d|O|l)/
    matches.each{m, d -> line = (line =~ "${m}").replaceFirst("r1"+d) }
    // Digit on the left
    matches  =  line =~ /resi(\d|OO|Ol|ll)/
    matches.each{m, d -> line = (line =~ "${m}").replaceFirst("r1"+d) }
    matches  =  line =~ /bdsi(\d|OO|Ol)/
    matches.each{m, d -> line = (line =~ "${m}").replaceFirst("h1"+d) }
    // Digit on the left
    matches  = line  =~ /(\d)i/
