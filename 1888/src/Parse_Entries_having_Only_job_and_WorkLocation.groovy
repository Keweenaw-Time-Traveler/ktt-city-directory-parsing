...
/*
        ...
        if(!name_parts[name_parts.length-1].equals("Co")&&!name_parts[name_parts.length-1].equals("CO")&&!name_parts[name_parts.length-1].equals("Bros")&&!name_parts[name_parts.length-1].equals("BROS")&&!name_parts[name_parts.length-1].equals("SON")&&!name_parts[name_parts.length-1].equals("Son")) {
            ...
            if (name_parts.size() >= 2 && name_parts.size() <= 3) {
                ...
                for (int i = 0; i <Jobs.size(); i++) {
                    if(line.split(',')[1].trim().contains(Jobs[i])&&line.indexOf(Jobs[i])==0) {
                        hasName = true;
        ...
        if(hasName) {
            ...
            if(!outJobLocation.equals(null)) {
                out = outLineNum + ",${outFirstName},${outLastName},${outMI},${outJrMrs},${outJob},${outJobLocation},${outT},${outStNum},${outStName},${outCity},${outBuilding}" + "\n"
                outCSVFile << out
            }
            else
            { if(outJob.equals(null))
                    outRejectFile<< outLineNum +" JOB NOT FOUND: "+savedLine+"\n"
                else
                outRejectFile<< outLineNum +" JOB LOCATION NOT FOUND: "+savedLine+"\n"
